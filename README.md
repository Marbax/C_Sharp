...
#### Одно на выбор из двух : 

> - Простое - разработать структурный тип для линейного уравнения с ексепшинами
> - Метод Крамера(матрици) проще чем Гауса для высчитывания линейного уравнения
> - Нужно только выкинуть исключение (не нужно обрабатывать или ловить как то)
> - Сплитом разбивать строку , если бульше двух чисел - выкидывать эксепшн

> - Написать программу позволяющий просматривать матчи до текущего момента и проводить гонки
> - 3 разных класса
> - Пилот, команда ,сезон 
> - Любые методы можно добавлять ,поля нельзя 
> - Нужны и генерировать и обрабатывать исключения (чтоб приложение не вылетало)
> - Почитать что такое формула 1
> - Настоящие команды , как очки распределяются 
> - Должна быть полноценная программа
> - Например исключения : Гет пилот по позиции , может не быть позицци, Пилот получить очки ,а пилота не существует
> - Таблици не существует ,она виртуальна, берутся все пилоты ,формируется временный массив, сортируется и возвращается ,хранить отдельно не нужно
> - места пилотов определяются суммой очков (все время пересчитывается )
